...
async function fix() {
  ...
  const exec = async (sql: string, label: string) => {
    ...
    catch (e: any) { if (e.message.includes("already exists") || e.message.includes("duplicate")) { console.log("⏭️  Already up to date:", label); } else { console.log("❌", label, "→", e.message); } }
  };
...
fix().catch(e => { console.error("❌ Fatal error:", e.message); process.exit(1); });
