    ...
    <div className="space-y-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="grid grid-cols-3 gap-2 text-sm">
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
                              ...
                              <p className="font-bold text-primary">{sp.price.toLocaleString("en-US")} PHP</p>
                            </div>
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
                              <p className="text-muted-foreground text-xs">Return</p>
                              <p className="font-bold text-green-600">{sp.returnAmount.toLocaleString("en-US")} PHP</p>
                            </div>
                            <div className="bg-secondary/50 rounded-lg p-2 text-center">
                              <p className="text-muted-foreground text-xs">Duration</p>
                              <p className="font-bold">{sp.lockDays} days</p>
                            </div>
                          </div>
                          {sp.launchDate && (
        ...
        <TabsContent value="stakings" className="space-y-4 mt-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            <div className="bg-secondary rounded-xl p-3 text-center">
              ...
              <p className="font-bold text-orange-600">{totalStaked.toLocaleString("en-US")} PHP</p>
            </div>
            <div className="bg-secondary rounded-xl p-3 text-center">
              <p className="text-xs text-muted-foreground">Total released</p>
              <p className="font-bold text-green-600">{totalReleased.toLocaleString("en-US")} PHP</p>
            </div>
          </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant={s.status === "released" ? "default" : "secondary"} className="text-xs">
                            {s.status === "released" ? "Released" : "In progress"}
                          </Badge>
                        </div>
                        <p className="text-xs text-muted-foreground">{s.user.phone} · {s.product.name}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {s.amountPaid.toLocaleString("en-US")} PHP → {s.returnAmount.toLocaleString("en-US")} PHP · Release: {formatDate(s.releaseDate)}
                        </p>
                      </div>
      ...
      <Dialog open={showForm} onOpenChange={closeForm}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editTarget ? "Edit staking product" : forms.length > 1 ? `Add ${forms.length} staking products` : "Add a staking product"}
            </DialogTitle>
          </DialogHeader>
              ...
              <div key={i} className={`space-y-3 ${forms.length > 1 ? "border border-border rounded-xl p-4" : ""}`}>
                ...
                <div>
                  <label className="text-sm font-medium flex items-center gap-2">
                    ...
                    Launch date <span className="text-muted-foreground font-normal">(optional — if scheduled, users can see the product but cannot buy it before launch)</span>
                  </label>
