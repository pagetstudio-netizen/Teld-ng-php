      ...
      <CardContent className="p-3">
        ...
        <div className="mt-2 pt-2 border-t">
          <p className="text-sm font-medium text-primary">
             Total invested: {formatCurrency(member.totalInvested, member.country)}
          </p>
            ...
            <div className="mt-1">
              ...
              {member.products.map((p, i) => (
                <p key={i} className="text-xs">
                   - {p.productName} ({formatCurrency(p.productPrice, member.country)})
            ...
            <Tabs defaultValue="level1" className="w-full">
                ...
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                       {formatCurrency(teamData.totalLevel1Invested, teamData.level1[0]?.country || "PH")}
                    </p>
              ...
              <TabsContent value="level2" className="mt-4">
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                       {formatCurrency(teamData.totalLevel2Invested, teamData.level2[0]?.country || "PH")}
                    </p>
              ...
              <TabsContent value="level3" className="mt-4">
                <Card className="mb-3">
                  <CardContent className="p-3 text-center">
                    <p className="text-lg font-bold text-primary">
                       {formatCurrency(teamData.totalLevel3Invested, teamData.level3[0]?.country || "PH")}
                    </p>
                      ...
                      <SelectContent>
                        {products?.filter(p => !p.isFree).map((product) => (
                          <SelectItem key={product.id} value={product.id.toString()}>
                             {product.name} - {formatCurrency(product.price, selectedUser.country)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                        ...
                        <div key={up.id} className={`flex items-center justify-between p-2 rounded-lg ${up.isActive ? "bg-green-500/10 border border-green-500/20" : "bg-secondary"}`}>
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                               {formatCurrency(up.productPrice, selectedUser.country)} - Day {up.daysClaimed}/{up.totalCycle}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {isSuperAdmin && !selectedUser.isSuperAdmin && (
                    <div className="col-span-2 space-y-2">
                      ...
                      >
                        <Shield className="w-4 h-4 mr-2" />
                        {selectedUser.isAdmin ? "Remove admin" : "Appoint admin"}
                      </Button>
                      {selectedUser.isAdmin && (
                        <Button
