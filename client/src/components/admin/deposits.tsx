...
import { Check, X, Ban, Search, Loader2, ImageIcon, MessageSquare, Clipboard } from "lucide-react";
...
export default function AdminDeposits() {
  ...
  const [copyingSeapayId, setCopyingSeapayId] = useState<number | null>(null);
  ...
  const copySeapayRequest = async (deposit: DepositWithUser) => {
    setCopyingSeapayId(deposit.id);
    try {
      const response = await fetch(`/api/admin/deposits/${deposit.id}/seapay-request`, { credentials: "include" });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || "Unable to prepare the SeaPay request");
      await navigator.clipboard.writeText(data.text);
      toast({
        title: "SeaPay request copied",
        description: `Reference: ${deposit.seapayReference}`,
      });
    } catch (error: any) {
      toast({ title: "Copy failed", description: error.message, variant: "destructive" });
    } finally {
      setCopyingSeapayId(null);
    }
  };
    ...
    <div className="space-y-4">
      {pendingCount > 0 && (
        <div className="bg-yellow-50 dark:bg-yellow-950 border border-yellow-200 dark:border-yellow-800 rounded-xl p-3 flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-yellow-700 dark:text-yellow-300">
            {pendingCount} pending deposit{pendingCount > 1 ? "s" : ""} awaiting approval
          </p>
        </div>
      )}
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          filteredDeposits.map((deposit) => {
            const isSeapay = Boolean(deposit.seapayReference);
            const isManual = !isSeapay && (!!(deposit as any).paymentNumberId || !!(deposit as any).channelName);
            return (
                ...
                <CardContent className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-xl p-3">
                    <div>
                      <p className="text-muted-foreground text-xs">Amount</p>
                       <p className="font-bold text-lg text-primary">{formatCurrency(deposit.amount, deposit.user.country)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="font-medium text-xs">
                        {new Date(deposit.createdAt).toLocaleDateString("en-US", { day: "2-digit", month: "2-digit", year: "numeric" })}
                        {" "}
                        {new Date(deposit.createdAt).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" })}
                      </p>
                    </div>
                    ...
                    {isSeapay && (
                      <div className="col-span-2 rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-900 dark:bg-blue-950/40">
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            <p className="text-muted-foreground text-xs">SeaPay merchant order</p>
                            <p className="truncate font-mono font-medium">{deposit.seapayReference}</p>
                            {deposit.seapayOrderId && (
                              <p className="truncate text-xs text-muted-foreground">SeaPay order: {deposit.seapayOrderId}</p>
                            )}
                          </div>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => copySeapayRequest(deposit)}
                            disabled={copyingSeapayId === deposit.id}
                            data-testid={`button-copy-seapay-${deposit.id}`}
                          >
                            {copyingSeapayId === deposit.id
                  ...
                  {(deposit.status === "pending" || deposit.status === "processing") && (
                    <div className="flex gap-2">
                      ...
                      >
                        {processingId === deposit.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-1" />Approve</>}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
      ...
      <Dialog open={!!screenshotModal} onOpenChange={() => setScreenshotModal(null)}>
        <DialogContent className="max-w-lg">
          ...
          {screenshotModal && (
            <img src={screenshotModal} alt="Screenshot" className="w-full rounded-xl object-contain max-h-[70vh]" />
          )}
        </DialogContent>
      </Dialog>
    </div>
