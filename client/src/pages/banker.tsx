...
import { Search, Check, X, LogOut, Loader2, ArrowDownCircle, ArrowUpCircle, History, ImageIcon, RefreshCw, Send, Copy } from "lucide-react";
...
export default function BankerPage() {
  ...
  const [seapayProcessingId, setSeapayProcessingId] = useState<number | null>(null);
  ...
  const seapayMutation = useMutation({
    mutationFn: async (id: number) => {
      setSeapayProcessingId(id);
      const res = await apiRequest("POST", `/api/banker/withdrawals/${id}/seapay`, {});
      if (!res.ok) { const data = await res.json(); throw new Error(data.message || "SeaPay payout error"); }
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/banker/withdrawals"] });
    ...
    onError: (e: any) => toast({ title: "SeaPay error", description: e.message, variant: "destructive" }),
    onSettled: () => setSeapayProcessingId(null),
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-50 bg-primary text-primary-foreground px-4 py-3 flex items-center justify-between shadow-md">
        <div>
          <h1 className="text-lg font-bold">Banker workspace</h1>
          <p className="text-xs opacity-80">
            {user?.fullName} · {user?.isAdmin ? "All countries" : user?.country === "NG" ? "Nigeria banker · process withdrawals in NGN" : "Philippines banker · process withdrawals"}
          </p>
        </div>
      ...
      <div className="px-4 pb-8">
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-lg p-3 mb-3">
                        <div>
                          ...
                          <p className="font-bold text-lg text-primary">{formatCurrency(Number(deposit.amount), deposit.user.country)}</p>
                        </div>
                        <div>
                        ...
                        <div className="col-span-2">
                          ...
                          <p className="text-sm">{new Date(deposit.createdAt).toLocaleString("en-US")}</p>
                        </div>
                      </div>
                       {(deposit as any).screenshot && (
                         <div className="mb-3">
                           ...
                           <button
                             ...
                             className="w-full overflow-hidden rounded-xl border border-border transition-colors hover:border-primary"
                             data-testid={`button-screenshot-${deposit.id}`}
                           >
                             <img src={(deposit as any).screenshot} alt="Payment screenshot" className="max-h-40 w-full bg-secondary/30 object-contain" />
                           </button>
                         </div>
                       )}
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm bg-secondary/50 rounded-lg p-3 mb-3">
                        <div>
                          ...
                          <p className="font-bold text-lg">{formatCurrency(Number(w.amount), w.user.country)}</p>
                        </div>
                        <div>
                          <p className="text-muted-foreground text-xs">Net amount</p>
                          <p className="font-bold text-lg text-primary">{formatCurrency(Number(w.netAmount), w.user.country)}</p>
                        </div>
                        <div>
                        ...
                        <div>
                          <p className="text-muted-foreground text-xs">Date</p>
                          <p className="text-sm">{new Date(w.createdAt).toLocaleString("en-US")}</p>
                        </div>
                      </div>

                       {w.status === "pending" && (
                         <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                           {w.seapayPayoutAvailable && (
                             <Button
                               variant="outline"
                               size="sm"
                               className="border-cyan-600 text-cyan-700 hover:bg-cyan-50"
                               onClick={() => seapayMutation.mutate(w.id)}
                               disabled={seapayProcessingId === w.id || withdrawalMutation.isPending}
                               data-testid={`button-seapay-withdrawal-${w.id}`}
                             >
                               {seapayProcessingId === w.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Send className="w-4 h-4 mr-1" />Send to SeaPay</>}
                             </Button>
                           )}
                          <Button
                            className="flex-1 bg-green-600 hover:bg-green-700 text-white"
                          ...
                          >
                             {withdrawalMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <><Check className="w-4 h-4 mr-1" />Process manually</>}
                          </Button>
                          <Button
                            variant="destructive"
                            className="flex-1"
                       ...
                       {w.seapayPayoutReference && (
                         <div className="flex items-center gap-2">
                           <p className="text-xs text-muted-foreground font-mono truncate flex-1">
                             SeaPay: {w.seapayPayoutReference}
                           </p>
                           <Button
                             size="sm"
                             variant="ghost"
                             onClick={() => copySeapayRequest(w.id)}
                             data-testid={`button-copy-seapay-withdrawal-${w.id}`}
                           >
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{date.toLocaleString("en-US")}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className={`font-bold ${type === "deposit" ? "text-green-600" : "text-orange-600"}`}>
                            {type === "deposit" ? "+" : "-"}{formatCurrency(Number(type === "deposit" ? item.amount : item.netAmount), item.user.country)}
                          </p>
        ...
        <Dialog open={!!screenshotModal} onOpenChange={() => setScreenshotModal(null)}>
          <DialogContent className="max-w-lg">
            ...
            {screenshotModal && (
              <img src={screenshotModal} alt="Payment screenshot" className="max-h-[70vh] w-full rounded-xl object-contain" />
            )}
          </DialogContent>
        </Dialog>
      </div>
