        ...
        <div className="flex items-start justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Registered on {new Date(member.createdAt).toLocaleDateString("en-US")}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-primary">
                 {formatCurrency(member.totalInvested, member.country)}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-1">
              {member.products.map((p, i) => (
                <div key={i} className="flex items-center justify-between text-sm bg-secondary rounded-lg px-3 py-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{formatCurrency(p.productPrice, member.country)}</span>
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className="text-xs text-muted-foreground">Total investi</p>
             <p className="text-xl font-bold text-primary">{formatCurrency(total, user?.country || "PH")}</p>
          </div>
        </div>
      </CardContent>
    </Card>
            ...
            <Card className="mb-4">
              <CardContent className="p-4">
                ...
                <div className="mt-4 pt-4 border-t text-center">
                  <p className="text-xs text-muted-foreground">Total invested by the team</p>
                  <p className="text-2xl font-bold text-primary">
                    {formatCurrency(teamData.totalLevel1Invested + teamData.totalLevel2Invested + teamData.totalLevel3Invested, user?.country || "PH")}
                  </p>
                </div>
              </CardContent>
            </Card>
